...
class PALGame:
    ...
    def reset(self):
        ...
        self.stateId = np.random.randint(0, len(self.xUnlabeled))
        return self.createState()


    def createState(self):
        alFeatures = self.getClassifierFeatures(self.xUnlabeled[self.stateId])
        return alFeatures
    ...
    def getClassifierFeatures(self, x):
        eps = 1e-7
        # prediction metrics
        pred = self.classifier(x.unsqueeze(0)).detach()
        two_highest, _ = pred.topk(2, dim=1)

        f1 = torch.Tensor([self.currentTestF1])
    ...
    def step(self, action):
        ...
        if action == 1:
            ...
            ret = addDatapointToPool(self.xLabeled, self.yLabeled,
                                     ...
                                     self.perClassIntances, self.stateId)
    ...
    def checkDone(self):
        done = self.nInteractions >= self.max_interactions # max interactions
