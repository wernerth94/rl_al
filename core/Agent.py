...
class BaseAgent:
    def __init__(self, state_space, action_space, clipped=True, gamma=0.9,
                 weight_copy_interval=500, n_hidden=10, lr=0.01, weight_decay=0.0):
        ...
        self.model = self.network(state_space, action_space, n_hidden=n_hidden)
        self.model = self.model.to(self.device)
        self.target_model = self.network(state_space, action_space, n_hidden=n_hidden)
        self.target_model.load_state_dict(self.model.state_dict())
        self.target_model = self.target_model.to(self.device)
        self.optimizer = optim.Adam(self.model.parameters(), lr=lr, weight_decay=weight_decay)
        self.loss = nn.SmoothL1Loss()

    def network(self, state_space, action_space, n_hidden)->nn.Module:
        raise NotImplementedError()

    def _forward(self, inputs, model='main')->torch.Tensor:
    ...
    def predict(self, inputs, model='main', *args, **kwargs):
        # wrapper for action selection
        # return self._forward(inputs, model)
        raise NotImplementedError()

    def _apply_update(self, total_loss, lr)->None:
        if lr:
...
class DDVN(BaseAgent):
    ...
    def __init__(self, stateSpace, clipped=True, gamma=0.99,
                 lr=0.001, n_hidden=10, weight_copy_interval=3, weight_decay=0.0):
        super().__init__(stateSpace, None, clipped, gamma, weight_copy_interval, n_hidden, lr, weight_decay)
    ...
    def predict(self, inputs, greed=0.0, model='main'):
        v = self._forward(inputs, model)
        eps = np.random.rand()
        if greed <= 0 or eps > greed:
            action = torch.argmax(v, dim=0)
        else:
            action = torch.tensor([np.random.randint(len(v))])
        return v, action
...
class DDQN(BaseAgent):
    ...
    def predict(self, inputs, greed=1, model='main'):
        ...
        if greed <= 0 or eps > greed:
            ...
            action = torch.tensor(np.random.randint(self.action_space, size=len(q)))
        return q, action
...
class Baseline_BvsSB(Baseline_Heuristic):
